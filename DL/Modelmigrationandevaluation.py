...
X_train,X_test,y_trainfinal,y_testfinal = prepData.SplitSample(SamplePathes,finalUseLabel,0.9,finalclass);
LoadCNNmodel = load_model(r'D:\lunwen3Localresult\zongxian\ZXCNN1D2Class3times2band20ext3.h5');
# 利用测试集进行精度评价，并利用模型对所有数据进行分类

resultLabel, score, lenthTest, Conv2DCNNHHmat = ModelCFP.PredictLabelConv1D( ImageData, X_test,y_testfinal,LoadCNNmodel,Rows,Cols);
prepData.Draw2orMoreClassResult(resultLabel, 'CNNTEMP');
print(score)
print(lenthTest)
print(Conv2DCNNHHmat)
sio.savemat(r'D:\lunwen3TranferResult\zongxian2banan\zonxian2bananConv1D.mat', {'DLMC': resultLabel});
